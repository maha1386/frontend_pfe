...
export default function SignaturePage() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        const data = await verifierToken(token);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await enregistrerSignature(token, base64);
      setState("success");
    } catch (err) {
  ...
  if (state === "invalid") {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-6 text-center">
        ...
        <p className="text-gray-500 mt-2 text-sm">Ce lien de signature est invalide ou a déjà été utilisé.</p>
      </div>
    );
  }
