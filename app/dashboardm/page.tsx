...
import { useOnboardingProgress } from "../hooks/dashboard/useOnboardingProgress";
...
function OnboardingProgress() {
  const { items, currentPage, lastPage, total, loading, fetchPage } =
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* ── Liste ── */}
      {items.map((item) => {
        const pct      = item.onboarding_progress;
        ...
        return (
          <div key={item.id} className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-slate-400 w-7 text-right">
              {pct}%
            </span>
          </div>
        );
