...
/**
 * Convertit experience (liste d'objets OU string) → texte lisible pour l'affichage
 */
const formatExperience = (exp: ExperienceEntry[] | string | any): string => {
  if (!exp) return "";
  ...
  if (Array.isArray(exp)) {
    return exp
      .map((e: ExperienceEntry) => {
        ...
        else if (e.duration_label) lines.push(e.duration_label);
        if (e.bullets?.length) lines.push(...e.bullets.map((b: string) => `• ${b}`));
        return lines.join("\n");
      })
      .join("\n\n");
  }
...
/**
 * Supprime le mot-clé OCR parasite "EDUCATION" en début de chaîne
 */
const cleanEducation = (text: string): string =>
  text.replace(/^EDUCATION[\s:–\-]*/i, "").trim();

/**
 * Mots-clés indiquant que la zone "certificat" a été mal identifiée par YOLO
 */
const CERT_NOISE = ["website", "linkedin", "address", "summary", "phone", "email"];

const isCertNoise = (text: string): boolean => {
  const lower = text.toLowerCase();
  return CERT_NOISE.some((kw) => lower.includes(kw));
};
...
export default function CvUploadPage() {
  ...
  // Texte éditable de l'expérience (string formatée pour le textarea)
  const [experienceText, setExperienceText] = useState("");
  // Texte éditable de la formation (nettoyé)
  ...
  // Initialiser les champs texte dès que cvData arrive
  useEffect(() => {
    if (cvData) {
      setExperienceText(formatExperience(cvData.experience));
      setEducationText(cleanEducation(cvData.education || ""));
    }
  }, [cvData]);
  ...
  const handleFile = (f: File) => {
    ...
    if (f.size > 100 * 1024 * 1024) {
      setErrorMsg("Le fichier ne doit pas dépasser 100 MB.");
      return;
    }
    setFile(f);
    setErrorMsg("");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Nettoyer le champ certificat côté front si YOLO a mal identifié la zone
      if (data.certificat && isCertNoise(data.certificat)) {
        data.certificat = "";
      }
  ...
  const handleValidate = async () => {
    ...
    // Construire le payload final avec les textes édités
    const payload: CvData = {
      ...cvData,
      experience: experienceText,   // toujours string au moment de la sauvegarde
    ...
    try {
      const res = await fetch("http://localhost:8000/api/cv/validate", {
        ...
        body: JSON.stringify({ cv_data: payload }),
      });
      if (!res.ok) throw new Error("Erreur sauvegarde");

      const user = JSON.parse(localStorage.getItem("user") || "{}");
      user.cv_data = payload;
            ...
            >
              ...
              {file ? (
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ fontWeight: 600, color: "#334155", margin: "0 0 4px", fontSize: "14px" }}>
        ...
        >
          ...
          <p style={{ fontSize: "13px", color: "#64748b", margin: "0 0 32px", lineHeight: 1.6 }}>
            {status === "uploading"
              ...
              : "YOLO détecte les zones · PaddleOCR lit le texte"}
          </p>
        ...
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div className="cv-card" style={{ padding: "24px" }}>
                ...
                <div style={{ marginTop: "16px" }}>
                  <TagsEdit
                    tags={cvData.skills}
                    onChange={(tags) => setCvData({ ...cvData, skills: tags })}
                    color="#7c3aed"
                  />
                </div>
              </div>

              {/* Certifications — masquées si bruit YOLO */}
              {cvData.certificat && !isCertNoise(cvData.certificat) && (
                <div className="cv-card" style={{ padding: "24px" }}>
                  <SectionHeader icon={<Award size={14} />} title="Certifications" color="#d97706" />
                  <div style={{ marginTop: "16px" }}>
                    <TextAreaEdit
                      value={cvData.certificat}
                      onChange={(v) => setCvData({ ...cvData, certificat: v })}
                      rows={4}
                    />
                  </div>
                </div>
                ...
                <div className="cv-card" style={{ padding: "24px" }}>
                  ...
                  <div style={{ marginTop: "16px" }}>
                    <TagsEdit
                      ...
                      onChange={(tags) => setCvData({ ...cvData, interests: tags })}
                      color="#0891b2"
                    />
                  </div>
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              {/* Expérience — affichée via experienceText (string formatée) */}
              <div className="cv-card" style={{ padding: "24px" }}>
                <SectionHeader icon={<Briefcase size={14} />} title="Expérience professionnelle" color="#2563eb" />
                <div style={{ marginTop: "16px" }}>
                  <TextAreaEdit
                    value={experienceText}
                    onChange={(v) => setExperienceText(v)}
                    rows={10}
                  />
                </div>
              </div>

              {/* Formation — affichée via educationText (nettoyé) */}
              <div className="cv-card" style={{ padding: "24px" }}>
                <SectionHeader icon={<GraduationCap size={14} />} title="Formation" color="#059669" />
                <div style={{ marginTop: "16px" }}>
                  <TextAreaEdit
                    value={educationText}
                    onChange={(v) => setEducationText(v)}
                    rows={6}
                  />
                </div>
              </div>
              ...
              {cvData.projects && cvData.projects.length > 0 && (
                <div className="cv-card" style={{ padding: "24px" }}>
                  ...
                  <div style={{ marginTop: "16px" }}>
                    <TagsEdit
                      tags={cvData.projects}
                      onChange={(tags) => setCvData({ ...cvData, projects: tags })}
                      color="#7c3aed"
                    />
                  </div>
                </div>
              ...
              {/* Bouton valider */}
              {errorMsg && (
                <p style={{ color: "#ef4444", fontSize: "13px", textAlign: "center" }}>{errorMsg}</p>
              )}

              <button
                className={`cv-btn-primary ${saving ? "disabled" : ""}`}
          ...
          <p style={{ textAlign: "center", fontSize: "11px", color: "#cbd5e1", margin: "28px 0 0" }}>
            © 2026 Maison du Web
          </p>
        </div>
      </div>
      ...
      {steps.map((step, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <span
            style={{
              fontSize: "12px",
              color: step.active ? "#4f46e5" : step.done ? "#10b981" : "#94a3b8",
...
function FieldEdit({
  ...
  return (
    <div>
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} className="cv-input" />
    </div>
  );
}
